...
function stag_comment( $comment, $args, $depth ) {
	...
	$args['reply_text']  = '<i class="stag-icon icon-reply"></i>';
	...
	<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?>">
		...
		<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
			...
			<div class="navigation-links grid-3">
				...
				<span class="nav-next"><?php previous_posts_link( __( '<span class="stag-icon icon-angle-left"></span>', 'stag' ) ); ?></span>
				<?php endif; ?>

				<?php if ( get_next_posts_link() ) : ?>
				<span class="nav-previous"><?php next_posts_link( __( '<span class="stag-icon icon-angle-right"></span>', 'stag' ) ); ?></span>
				<?php endif; ?>
			</div>
...
function stag_posted_on() {
	...
	if ( ! post_password_required() && ( comments_open() || '0' != get_comments_number() ) ) :
		echo ' with <span class="comments-link">';
